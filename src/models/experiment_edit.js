...
let model = {
  ...
  state: {
    ...
    experiment_edit_delete_data: null
  },
  ...
  effects: {
    ...
    *delete_experiment_edit({ payload }, sagaEffects) {
      const { call, put } = sagaEffects;
      let result = "";
      try {
        // 捕获异常
        result = yield call(
          experiment_edit_service.delete_experiment_edit,
          payload
        );
      } catch (e) {
        result = "";
      ...
      yield put({
        type: "delete_experiment_edit_result",
        payload: result
      });
      return 0;
    }
  ...
  reducers: {
    ...
    delete_experiment_edit_result(state, { payload: result }) {
      ...
      if ("error" != result) {
        try {
          ...
          let { status = "error_1" } = result;
          // 尝试写入 data
          obj.experiment_edit_delete_data = result.data;
          // 如果提取到 status 那么就 把 status 返回
          res_status = "error_1" == status ? -1 : status;
        } catch (e) {
          // 转换出错
      ...
      obj.experiment_edit_delete_time = new Date().getTime();

      // 1 检查 返回数据状态
      if (-1 == res_status) {
        // 发生严重错误
        obj.experiment_edit_delete_status = res_status;
        return obj;
      }
