...
let model = {
    ...
    state: {
        ...
        // 返回的数据
        project_filemanager_data: null,
        project_filemanager_send_json_file_status: -1,
        project_filemanager_send_json_file_time: 0,
        project_filemanager_send_json_file_data: null
    },
    ...
    effects: {
        ...
        *send_json_file({ payload }, sagaEffects) {
            const { call, put } = sagaEffects;
            let result = "";
            try {
                // 捕获异常
                result = yield call(
                    project_filemanager_service.send_json_file,
                    payload
                );
            } catch (e) {
                result = "";
    ...
    reducers: {
        ...
        send_json_file_result(state, { payload: result }) {
            // 尝试提取返回结果
            let res_status = -1;
            let obj = {};
            for (let i in state) {
            ...
            if ("error" != result) {
                try {
                    ...
                    let { status = "error_1" } = result;
                    // 尝试写入 data
                    obj.project_filemanager_send_json_file_data = result.data;
                    // 如果提取到 status 那么就 把 status 返回
                    res_status = "error_1" == status ? -1 : status;
                } catch (e) {
                    // 转换出错
            ...
            obj.project_filemanager_send_json_file_time = new Date().getTime();

            // 1 检查 返回数据状态
            if (-1 == res_status) {
                // 发生严重错误
                obj.project_filemanager_send_json_file_status = res_status;
                return obj;
            }
