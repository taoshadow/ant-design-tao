// src/pages/propro/standard_library/detail.js
// 公共标准库列表id 详情
...
const standard_library_state_to_props = state => {
  ...
  let {
    ...
    standard_library_detail_status,
    // 最新获取数据的时间戳
    standard_library_detail_time,
    standard_library_detail_data,

    // 生成肽段
    standard_library_detail_generate_time,
    standard_library_detail_generate_status,

    // 更新状态
    standard_library_detail_aggregate_status,
    standard_library_detail_aggregate_time,

    // 删除肽段
    standard_library_detail_delete_pseudopeptides_status,
    standard_library_detail_delete_pseudopeptides_time,

    // 删除库
    standard_library_detail_delete_status,
    standard_library_detail_delete_time
  } = state["standard_library_detail"];

  (obj.standard_library_detail_generate_time = standard_library_detail_generate_time),
    (obj.standard_library_detail_generate_status = standard_library_detail_generate_status),
    (obj.standard_library_detail_delete_time = standard_library_detail_delete_time),
    (obj.standard_library_detail_delete_status = standard_library_detail_delete_status),
    (obj.standard_library_detail_delete_pseudopeptides_time = standard_library_detail_delete_pseudopeptides_time),
    (obj.standard_library_detail_delete_pseudopeptides_status = standard_library_detail_delete_pseudopeptides_status),
    (obj.standard_library_detail_aggregate_time = standard_library_detail_aggregate_time),
    (obj.standard_library_detail_aggregate_status = standard_library_detail_aggregate_status),
    (obj.standard_library_detail_data = standard_library_detail_data),
    (obj.standard_library_detail_time = standard_library_detail_time),
    (obj.standard_library_detail_status = standard_library_detail_status);
...
const standard_library_dispatch_to_props = dispatch => {
  return {
    // 发起查询
    get_standard_library_detail: data => {
      const action = {
        type: "standard_library_detail/get_standard_library_detail",
        payload: data
      };
      dispatch(action);
    },
    ...
    aggregate: id => {
      const action = {
        type: "standard_library_detail/aggregate",
        payload: id
      };
      dispatch(action);
    },
    ...
    generate: obj => {
      const action = {
        type: "standard_library_detail/generate",
        payload: obj
      };
      dispatch(action);
    },
    ...
    delete_pseudopeptides: obj => {
      const action = {
        type: "standard_library_detail/delete_pseudopeptides",
        payload: obj
      };
      dispatch(action);
    },
    ...
    delete_standard_library_by_id: obj => {
      const action = {
        type: "standard_library_detail/delete_standard_library_by_id",
        payload: obj
      };
      dispatch(action);
    },
    ...
    set_state_newvalue: data => {
      const action = {
        type: "standard_library_detail/set_state_newvalue",
        payload: data
      };
      dispatch(action);
    }
...
class Standard_library_detail extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  // 查询详情数据
  query_list_detail = () => {
    // /standard_library/detail/5c6d4e59fc6f9e0b70702806
    ...
    // 主动发起查询请求
    this.props.get_standard_library_detail({ id: id });
    setTimeout(() => {
      // 写入 id
      this.setState({
  ...
  handle_standard_library_detail = () => {
    // 更新
    // 时间戳设置为 0
    this.props.set_state_newvalue({
      target: "standard_library_detail_time",
      value: 0
    });

    if (0 == this.props.standard_library_detail_status) {
      // 成功获取数据

      this.set_standard_library_detail_data();
    } else {
      // 数据获取失败
      setTimeout(() => {
        this.setState({
  ...
  set_standard_library_detail_data = () => {
    let { standard_library_detail_data: data } = this.props;
    // 提取关键参数
    let obj_data = {};
    ...
    */
    ...
    setTimeout(() => {
      this.setState({
        standard_library_detail_data: obj_data,
        standard_library_list_detail_status: 0
      });
    }, 80);
  };
  ...
  aggregate = () => {
    // 获取到当前页面的id
    let { standard_library_id: id = null } = this.state;
  ...
  // 重新统计irt校准库
  handle_standard_library_detail_aggregate = () => {
    // 更新
    // 时间戳设置为 0
    this.props.set_state_newvalue({
      target: "standard_library_detail_aggregate_time",
      value: 0
    });
    ...
    // 读取状态
    if (0 == this.props.standard_library_detail_aggregate_status) {
      // 更新成功
      setTimeout(() => {
        // 提示
        message.success(
  ...
  delete_pseudopeptides = () => {
    let { standard_library_id: id } = this.state;
    let { language } = this.props;
  ...
  handle_standard_library_detail_delete_pseudopeptides = () => {
    // 处理 删除肽段
    // 时间戳设置为 0
    this.props.set_state_newvalue({
      target: "standard_library_detail_delete_pseudopeptides_time",
      value: 0
    });
    ...
    // 读取状态
    if (0 == this.props.standard_library_detail_delete_pseudopeptides_status) {
      // 更新成功
      setTimeout(() => {
        // 提示
        message.success(
  ...
  delete_library_by_id_confirm = () => {
    ...
    this.props.delete_standard_library_by_id({
      id: id
    });
    this.setState({
      modal_visible: false
  ...
  // 执行删除库操作
  handle_standard_library_detail_delete = () => {
    // 处理删除标准库的结果
    // 时间戳归零
    this.props.set_state_newvalue({
      target: "standard_library_detail_delete_time",
      value: 0
    });
    let { standard_library_detail_delete_status, language } = this.props;

    if (0 == standard_library_detail_delete_status) {
      // 删除成功
      // 弹出删除成功
      setTimeout(() => {
        message.success(
      ...
      // 延时跳转到 irt 标准库列表页面
      setTimeout(() => {
        this.props.history.push("/standard_library/list");
      }, 800);
    } else {
      setTimeout(() => {
        // 删除失败
  ...
  handel_standard_library_detail_generate = () => {
    // 处理删除标准库的结果
    // 时间戳归零
    this.props.set_state_newvalue({
      target: "standard_library_detail_generate_time",
      value: 0
    });

    // 提取状态
    let { standard_library_detail_generate_status, language } = this.props;

    // 这里暂时只处理 生成肽段成功的情况
    if (0 == standard_library_detail_generate_status) {
      //
      // 成功生成伪肽段
      setTimeout(() => {
        // 提示
  ...
  render() {
    ...
    if (10000 < this.props.standard_library_detail_time) {
      // 资源有更新
      this.handle_standard_library_detail();
    }
    if (0 != this.state.standard_library_list_detail_status) {
      // 加载 ...
      return (
        <Fragment>
          <Row>
    ...
    if (10000 < this.props.standard_library_detail_aggregate_time) {
      // 重新统计 更新
      this.handle_standard_library_detail_aggregate();
    }

    if (10000 < this.props.standard_library_detail_generate_time) {
      // 传回更新肽段
      this.handel_standard_library_detail_generate();
    }

    if (10000 < this.props.standard_library_detail_delete_pseudopeptides_time) {
      // 删除肽段 更新
      this.handle_standard_library_detail_delete_pseudopeptides();
    }

    // 监控删除标准库
    if (10000 < this.props.standard_library_detail_delete_time) {
      this.handle_standard_library_detail_delete();
    }

    const detail_data = this.state.standard_library_detail_data;
      ...
      <div>
        ...
        >
          <Tooltip
            placement="topLeft"
            title={
              <FormattedHTMLMessage id="propro.standard_library_list_title" />
            }
          >
            <Link to="/standard_library/list">
              <img
                src={return_svg}
                style={{
                  height: "30px",
...
export default Standard_library_detail;
