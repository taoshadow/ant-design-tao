// src/pages/propro/standard_library_list.js
// 公共标准库
 ...
 * @UpdateTime          2019-9-6 09:59:08
 * @Archive             新版 查询并显示标准库列表
 */
...
/***********  standard_library_list View 初始化   ***************/
/***********  standard_library_list View 初始化   ***************/

const standard_library_list_state_to_props = state => {
  // 发送的对象
  let obj = {};
  ...
  let {
    ...
    // 设置更新的状态标记
    standard_library_list_set_public_status,
    standard_library_list_set_public_time
  } = state["standard_library_list"];

  (obj.standard_library_list_set_public_status = standard_library_list_set_public_status),
    (obj.standard_library_list_set_public_time = standard_library_list_set_public_time),
    (obj.standard_library_list_status = standard_library_list_status),
    (obj.standard_library_list_time = standard_library_list_time),
    (obj.standard_library_list_data = standard_library_list_data);

  return obj;
};

const standard_library_list_dispatch_to_props = dispatch => {
  return {
    // 更新触发器
    get_standard_library_list: () => {
      const action = {
        type: "standard_library_list/get_standard_library_list",
        payload: null
      };
      dispatch(action);
    },
    ...
    set_state_newvalue: data => {
      const action = {
        type: "standard_library_list/set_state_newvalue",
        payload: data
      };
      dispatch(action);
    },
    set_library_public_by_id: data => {
      const action = {
        type: "standard_library_list/set_library_public_by_id",
        payload: data
      };
      dispatch(action);
    }
...
/***********  standard_library_list View 初始化 end  ***************/

@connect(
  standard_library_list_state_to_props,
  standard_library_list_dispatch_to_props
)
class Standard_library_list extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      //   查询到的标准库数据
      ...
      standard_library_list_status: -1,
      // 请求失败再次发起请求的尝试次数
      standard_library_list_false_time: 5,
      search_text: ""
  ...
  handle_standard_library_list = () => {
    ...
    if (0 == this.props.standard_library_list_status) {
      ...
      setTimeout(() => {
        // 调用 添加更新数据函数
        this.set_standard_library_list_data();
        // 添加服务端数据
        this.setState({
          // 标记 成功
          standard_library_list_false_time: 5,
      ...
      // 2-判断是否需要再次发起请求
      if (0 >= standard_library_list_false_time) {
        console.error(
          "@Author:tangtao; 系统已终止运行,请重新刷新页面;2019-9-4 21:22:02 ",
      ...
      setTimeout(() => {
        this.setState({
          standard_library_list_false_time: standard_library_list_false_time--
        });
      }, 120);
  ...
  set_standard_library_list_data = () => {
    //   提取 model 层 传过来的数据
    const {
    ...
    } = this.props.standard_library_list_data;
    ...
    if (0 < len0) {
      ...
      for (let i in library_list) {
        ...
        (obj_temp.index = index + 1),
          (obj_temp.key = "standard_library_list_" + i),
          (obj_temp.name = name),
  ...
  set_this_public = (id = "", status) => {
    ...
    if (false == status && 5 < id.length) {
      ...
      // 提示
      message.loading(
        Languages[language]["propro.standard_library_list_set_public"] +
          " : " +
  ...
  handle_standard_library_list_set_public = () => {
    // 时间戳设置为 0
    this.props.set_state_newvalue({
      target: "standard_library_list_set_public_time",
      value: 0
    });

    // 提取返回状态
    let { standard_library_list_set_public_status, language } = this.props;

    if (0 == standard_library_list_set_public_status) {
      // 更新成功
      // 进入延时刷新
      setTimeout(() => {
  ...
  render() {
    // 定义 解析 配置 表格
    const standard_library_list_table_columns = [
      {
        //   序列号
        title: (
          <span
          ...
          >
            <FormattedHTMLMessage id="propro.standard_library_list_index" />
          </span>
        ),
        dataIndex: "index",
        key: "index"
      },
      {
        ...
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.standard_library_list_name" />
          </span>
        ),
        key: "name",
        ...
        render: list => {
          return (
            <Link to={"/standard_library/detail/" + list.id}>{list.name}</Link>
          );
        }
      },
      {
        ...
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.standard_library_list_id" />
          </span>
        ),
        dataIndex: "id",
        key: "id",
      ...
      },
      {
        ...
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.standard_library_list_is_public" />
          </span>
        ),
        dataIndex: "is_public",
        key: "is_public",
      ...
      },
      {
        ...
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.standard_library_list_protein_count" />
          </span>
        ),
      ...
      },
      {
        ...
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.standard_library_list_create_time" />
          </span>
        ),
      ...
      },
      {
        ...
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.standard_library_list_creator" />
          </span>
        ),
        dataIndex: "creator",
        key: "creator",
      ...
      },
      {
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.standard_library_list_operation" />
          </span>
        ),
        key: "action",
        render: list => {
          ...
          return (
            <Fragment key="2019-8-16 00:32:54">
              ...
              <Tooltip
                title={
                  <FormattedHTMLMessage id="propro.standard_library_list_detail" />
                }
              >
                <Link to={"/standard_library/detail/" + list.id}>
                  <button
                    type="button"
              ...
              <Tooltip
                title={
                  <FormattedHTMLMessage id="propro.standard_library_list_update" />
                }
              >
                <Link
                  to={"/standard_library/update/" + list.id + "_" + list.name}
                >
                  <button
                    type="button"
              ...
              <Tooltip
                title={
                  <FormattedHTMLMessage id="propro.standard_library_list_set_public" />
                }
              >
                <button
                  type="button"
    ...
    if (10000 < this.props.standard_library_list_time) {
      // 资源有更新
    ...
    if (10000 < this.props.standard_library_list_set_public_time) {
      this.handle_standard_library_list_set_public();
    }
    ...
    return (
      <div>
        ...
        >
          <Table
            size={"middle"}
            columns={standard_library_list_table_columns}
            pagination={{
              position: "bottom",
...
export default Standard_library_list;
