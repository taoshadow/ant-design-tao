// /src/pages/propro/library/standard_library_update.js
...
const standard_library_state_update_to_props = state => {
  let obj = {};
  ...
  let {
    ...
    standard_library_update_task_id,
  ...
  (obj.standard_library_query_task_time = standard_library_query_task_time),
    ...
    (obj.standard_library_update_task_id = standard_library_update_task_id);
  return obj;
};
...
const standard_library_update_dispatch_to_props = dispatch => {
  return {
    ...
    update_library_list_id_detail: data => {
      const action = {
        type: "standard_library_update/update_library_list_id_detail",
        payload: data
      };
      dispatch(action);
    },
...
@connect(
  standard_library_state_update_to_props,
...
class Public_library_update extends React.Component {
  constructor(props) {
    ...
    setTimeout(() => {
      this.get_standard_library_id_name();
    }, 400);
    // 配置 message
    message.config({
      top: 500,
      ...
      if (0 == this.props.standard_library_update_status) {
        ...
        setTimeout(() => {
          ...
          message.success(
            Languages[language]["propro.standard_library_update_title"] +
              " : " +
              Languages[language]["propro.standard_library_update_success"],
            2
          );
        }, 180);
          ...
          this.setState({
            query_task_interval: setInterval(() => {
              this.props.query_task_id({
                // 提取taskid
                task_id: this.props.standard_library_update_task_id
              });
            }, 2000)
          });
        }
        ...
        setTimeout(() => {
          ...
          message.error(
            Languages[language]["propro.standard_library_update_title"] +
              " : " +
              Languages[language]["propro.standard_library_update_failed"],
            4
          );
        }, 220);
      }
      ...
      <div>
        ...
        >
          ...
          >
            <Link
              to={
                "/library/standard_library/detail/" +
