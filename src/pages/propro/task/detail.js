...
const task_detail_state_to_props = state => {
  ...
  let {
    ...
    task_detail_time = 0,
    task_detail_data = {},
    delete_task_detail_by_id_time = 0,
    delete_task_detail_by_id_status = -1
  } = state["task_detail"];

  (obj.delete_task_detail_by_id_time = delete_task_detail_by_id_time),
    (obj.delete_task_detail_by_id_status = delete_task_detail_by_id_status),
...
const task_detail_dispatch_to_props = dispatch => {
  return {
    ...
    delete_task_detail_by_id: data => {
      const action = {
        type: "task_detail/delete_task_detail_by_id",
        payload: data
      };
      dispatch(action);
    },
    query_task_detail_by_custom: data => {
      const action = {
        type: "task_detail/query_task_detail_by_custom",
        payload: data
      };
      dispatch(action);
    }
...
class Task_detail extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      search_task_detail_by_templates: null,
  ...
  delete_task_detail_by_id = id => {
    // 弹出警告
    // 发起警告
    this.setState({
      delete_task_detail_modal_visible: true,
      task_detail_delete_id: id
    });
  };

  delete_task_detail_by_id_cancel = () => {
    this.setState({
      delete_task_detail_modal_visible: false,
      task_detail_delete_id: null
    });
  ...
  delete_task_detail_by_id_confirm = () => {
    //
    let { task_detail_delete_id: id = null } = this.state;
    ...
    if (null == id) {
      ...
      message.info(
        Languages[language]["propro.task_detail_operation_delete"] +
          " : " +
    ...
    setTimeout(() => {
      this.props.delete_task_detail_by_id({ id: id });
    }, 400);
  };

  // 处理删除任务结果
  handle_delete_task_detail_by_id = () => {
    // 时间戳先重新设置为 0
    this.props.set_state_newvalue({
      target: "delete_task_detail_by_id_time",
      value: 0
    });

    let {
      delete_task_detail_by_id_status: res_status = -100,
      language
    } = this.props;
    ...
    if (0 == res_status) {
      ...
      setTimeout(() => {
        this.handle_query_task_detail_by_custom();
      }, 4000);
    } else {
      // 删除失败
      setTimeout(() => {
  ...
  get_column_search_props = dataIndex => ({
    filterDropdown: ({
      ...
      <div style={{ padding: 8 }}>
        ...
        >
          <FormattedHTMLMessage id="propro.task_detail_search" />
        </Button>
        <Button
          ...
          style={{ width: 90 }}
        >
          <FormattedHTMLMessage id="propro.task_detail_reset" />
        </Button>
      </div>
    ),
    filterIcon: filtered => (
  ...
  handle_query_task_detail_by_custom = () => {
    // 用户发起自定义查询
    // 尝试提取自定义的参数
    let {
      load_percentage_value,
      search_task_detail_by_templates,
      page_size,
      total_page,
      task_templates,
      total_numbers
    ...
    if (
      0 >= (search_task_detail_by_templates + "").length ||
      "ALL" == search_task_detail_by_templates
    ) {
      // 数值为空 或者 指定 查询全部
      search_task_detail_by_templates = null;
    }
    let obj = {
      ...
      // 指定的查询参数
      search_task_detail_by_templates: search_task_detail_by_templates
    };

    this.props.query_task_detail_by_custom(obj);
    // 设置数据为不可用状态
    this.setState({
      task_detail_status: -1
    });
  ...
  render() {
    // 定义 解析 配置 表格
    const task_detail_table_columns = [
      {
        //   序列号
        title: (
          <span
          ...
          >
            <FormattedHTMLMessage id="propro.task_detail_index" />
          </span>
        ),
        dataIndex: "index",
        key: "index",
      ...
      },
      {
        ...
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.task_detail_name" />
          </span>
        ),
        key: "name",
        ...
        render: detail => {
          return (
            <Tooltip
              placement="top"
              title={
                <FormattedHTMLMessage id="propro.task_detail_link_detail" />
              }
            >
              <div
                style={{
              ...
              >
                {/* 跳转到详情页面 */}
                <Link to={"/task/detail/detail/" + detail.id}>
                  {detail.name}
                </Link>
              </div>
            </Tooltip>
      ...
      },
      {
        ...
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.task_detail_task_template" />
          </span>
        ),
      ...
      },
      {
        ...
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.task_detail_status" />
          </span>
        ),
        dataIndex: "status",
        key: "status",
      ...
      },
      {
        ...
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.task_detail_create_date" />
          </span>
        ),
      ...
      },
      {
        ...
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.task_detail_running_total_time" />
          </span>
        ),
      ...
      },
      {
        ...
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.task_detail_last_modified_date" />
          </span>
        ),
      ...
      },
      {
        ...
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.task_detail_creator" />
          </span>
        ),
        dataIndex: "creator",
        key: "creator",
      ...
      },
      {
        ...
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.task_detail_operation" />
          </span>
        ),
        dataIndex: "id",
        key: "operation_id",
        render: text => (
          ...
          <Tooltip
            title={
              <FormattedHTMLMessage id="propro.task_detail_operation_delete" />
            }
          >
            <button
              type="button"
              ...
              onClick={() => {
                this.delete_task_detail_by_id(text);
              }}
            >
              <img
                src={delete_svg}
    ...
    if (10000 < this.props.delete_task_detail_by_id_time) {
      this.handle_delete_task_detail_by_id();
    }
    ...
    let {
      ...
      task_templates,
      total_numbers,
      task_detail_query_time
    } = this.state;

    if (null == task_detail_data) {
      task_detail_data = [];
    }
    return (
      <div>
        ...
        <Row>
          <Col lg={24} xl={24} xxl={24}>
            ...
            >
              <Descriptions
                title={
                  <FormattedHTMLMessage id="propro.task_detail_table_info" />
                }
              >
                {/* 查询时间 */}
                <Descriptions.Item
                  label={
                    <FormattedHTMLMessage id="propro.task_detail_load_time" />
                  }
                >
                ...
                <Descriptions.Item
                  label={
                    <FormattedHTMLMessage id="propro.task_detail_load_numbers" />
                  }
                >
                  <span
                    className={
                      0 >= task_detail_data.length
                        ? styles.font_red_color
                        : styles.font_primary_color
                    }
                  >
                    {task_detail_data.length}
                  </span>
                </Descriptions.Item>
                ...
                <Descriptions.Item
                  label={
                    <FormattedHTMLMessage id="propro.task_detail_total_numbers" />
                  }
                >
                  <span
                    className={
              ...
              <Col span={24}>
                <div style={{ float: "left" }}>
                  ...
                  >
                    <FormattedHTMLMessage id="propro.task_detail_load_percentage" />
                    &nbsp;:
                  </div>
                  <div
                    style={{
              ...
              <Col span={24}>
                <div>
                  <FormattedHTMLMessage id="propro.task_detail_search_by_templates_title" />
                  &nbsp;:&nbsp;&nbsp;
                  <Select
                    showSearch
                    style={{ width: "300px" }}
                    defaultValue="ALL"
                    placeholder={
                      <FormattedHTMLMessage id="propro.task_detail_search_by_templates_title_prompt" />
                    }
                    disabled={null == task_templates}
                    onChange={this.change_search_task_detail_by_templates}
                  ...
                  <Button
                    ...
                    onClick={this.handle_query_task_detail_by_custom}
                  >
                    <span>
                      &nbsp;
                      <FormattedHTMLMessage id="propro.task_detail_search" />
            ...
            >
              <Table
                size={"middle"}
                columns={task_detail_table_columns}
                pagination={{
                  position: "top",
                  hideOnSinglePage: true,
                  defaultPageSize: 100
                }}
                dataSource={task_detail_data}
              />
            </Col>
          </Col>
        </Row>
