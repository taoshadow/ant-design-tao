// src/pages/propro/public_irt_library/list.js
...
const public_irt_standard_library_state_to_props = state => {
  ...
  let {
    ...
    public_irt_standard_library_list_data = {},
    // 设置更新的状态标记
    public_irt_standard_library_list_set_public_status,
    public_irt_standard_library_list_set_public_time
  } = state["public_irt_standard_library_list"];

  (obj.public_irt_standard_library_list_set_public_status = public_irt_standard_library_list_set_public_status),
    (obj.public_irt_standard_library_list_set_public_time = public_irt_standard_library_list_set_public_time),
    (obj.public_irt_standard_library_list_status = public_irt_standard_library_list_status),
    (obj.public_irt_standard_library_list_time = public_irt_standard_library_list_time),
    (obj.public_irt_standard_library_list_data = public_irt_standard_library_list_data);
...
const public_irt_standard_library_dispatch_to_props = dispatch => {
  return {
    // 更新触发器
    get_public_irt_standard_library_list: () => {
      const action = {
        type:
          "public_irt_standard_library_list/get_public_irt_standard_library_list",
        payload: null
      };
      dispatch(action);
    },
    set_state_newvalue: data => {
      const action = {
        type: "public_irt_standard_library_list/set_state_newvalue",
        payload: data
      };
      dispatch(action);
    },
    set_library_public_by_id: data => {
      const action = {
        type: "public_irt_standard_library_list/set_library_public_by_id",
        payload: data
      };
      dispatch(action);
    }
...
class Irt_standard_library_list extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      public_irt_standard_library_list_status: -1,
      // 请求失败再次发起请求的尝试次数
      public_irt_standard_library_list_false_time: 5,
      search_text: ""
      //   language: this.props.language
    };

    // 查询 public_irt 列表
    this.props.get_public_irt_standard_library_list();
  ...
  handle_public_irt_standard_library_list = () => {
    // 时间戳设置为 0
    this.props.set_state_newvalue({
      target: "public_irt_standard_library_list_time",
      value: 0
    });

    // 检查状态
    if (0 == this.props.public_irt_standard_library_list_status) {
      // 数据获取成功
      setTimeout(() => {
        // 调用 添加更新数据函数
        this.change_public_irt_standard_library_list_data();
        // 添加服务端数据
        this.setState({
          // 标记 成功
          public_irt_standard_library_list_false_time: 5,
          // 标记数据为可用的状态
          public_irt_standard_library_list_status: 0
        });
      }, 200);
    } else {
      // 数据获取失败
      ...
      let { public_irt_standard_library_list_false_time } = this.state;
      // 2-判断是否需要再次发起请求
      if (0 >= public_irt_standard_library_list_false_time) {
        console.error(
      ...
      setTimeout(() => {
        this.setState({
          public_irt_standard_library_list_false_time: public_irt_standard_library_list_false_time--
        });
      }, 120);
  ...
  change_public_irt_standard_library_list_data = () => {
    //   提取 model 层 传过来的数据
    const {
    ...
    } = this.props.public_irt_standard_library_list_data;
    ...
    if (0 < len0) {
      ...
      for (let i in library_list) {
        ...
        (obj_temp.index = index + 1),
          (obj_temp.key = "public_irt_library_list_" + i),
          (obj_temp.name = name),
    ...
    this.setState({
      public_irt_standard_library_list_data: obj_data
    });
  ...
  get_column_search_props = dataIndex => ({
    filterDropdown: ({
      ...
      <div style={{ padding: 8 }}>
        ...
        >
          <FormattedHTMLMessage id="propro.public_irt_standard_library_list_search" />
        </Button>
        <Button
          ...
          style={{ width: 90 }}
        >
          <FormattedHTMLMessage id="propro.public_irt_standard_library_list_reset" />
        </Button>
      </div>
    ),
    filterIcon: filtered => (
  ...
  set_this_public = (id = "", status) => {
    ...
    if (false == status && 5 < id.length) {
      ...
      message.loading(
        Languages[language][
          "propro.public_irt_standard_library_list_set_public"
        ] +
          " : " +
  ...
  handle_public_irt_standard_library_list_set_public = () => {
    // 时间戳设置为 0
    this.props.set_state_newvalue({
      target: "public_irt_standard_library_list_set_public_time",
      value: 0
    });
    ...
    if (0 == public_irt_standard_library_list_set_public_status) {
      // 更新成功
      // 进入延时刷新
      setTimeout(() => {
        this.setState({ public_irt_standard_library_list_status: -1 });
      }, 800);
      setTimeout(() => {
  ...
  render() {
    // 定义 解析 配置 表格
    const public_irt_standard_library_list_table_columns = [
      {
        //   序列号
        title: (
          <span
          ...
          >
            <FormattedHTMLMessage id="propro.public_irt_standard_library_list_index" />
          </span>
        ),
        dataIndex: "index",
        key: "index"
      },
      {
        ...
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.public_irt_standard_library_list_name" />
          </span>
        ),
        key: "name",
        ...
        render: list => {
          return (
            <Link to={"/library/public_irt_standard_library/detail/" + list.id}>
              {list.name}
            </Link>
          );
        }
      },
      {
        ...
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.public_irt_standard_library_list_id" />
          </span>
        ),
        dataIndex: "id",
        key: "id",
      ...
      },
      {
        ...
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.public_irt_standard_library_list_is_public" />
          </span>
        ),
        dataIndex: "is_public",
        key: "is_public",
      ...
      },
      {
        ...
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.public_irt_standard_library_list_protein_count" />
          </span>
        ),
      ...
      },
      {
        ...
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.public_irt_standard_library_list_create_time" />
          </span>
        ),
      ...
      },
      {
        ...
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.public_irt_standard_library_list_creator" />
          </span>
        ),
        dataIndex: "creator",
        key: "creator",
      ...
      },
      {
        title: (
          ...
          >
            <FormattedHTMLMessage id="propro.public_irt_standard_library_list_operation" />
          </span>
        ),
        key: "action",
        render: list => {
          ...
          return (
            <Fragment key="2019-8-16 00:32:54">
              ...
              <Tooltip
                title={
                  <FormattedHTMLMessage id="propro.public_irt_standard_library_list_detail" />
                }
              >
                <Link to={"/public_irt_standard_library/detail/" + list.id}>
                  <button
                    type="button"
    ...
    if (10000 < this.props.public_irt_standard_library_list_time) {
      // 资源有更新
      this.handle_public_irt_standard_library_list();
    }

    if (0 != this.state.public_irt_standard_library_list_status) {
      return (
        <Fragment>
          <Row>
            <Col
    ...
    if (10000 < this.props.public_irt_standard_library_list_set_public_time) {
      this.handle_public_irt_standard_library_list_set_public();
    }
    ...
    return (
      <div>
        ...
        >
          <Table
            size={"middle"}
            columns={public_irt_standard_library_list_table_columns}
            pagination={{
              position: "bottom",
              hideOnSinglePage: true
            }}
            dataSource={this.state.public_irt_standard_library_list_data}
          />
        </div>
      </div>
    );
